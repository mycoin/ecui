/*Copyright 2014 Baidu Inc. All rights reserved.*/var ecui;
(function() {
...
(function() {
    ...
    var indexOf = array.indexOf = function(list, obj) {
         ...
         */
        empty = dom.empty = function(element) {
         ...
         */
        setStyles = dom.setStyles = function(el, styles) {
         ...
         */
        format = string.format = function(source, opts) {
         ...
         * @remark
         * 截取过程中，遇到半个汉字时，向下取整。
         * @see ecui.string.getByteLength
         *
         * @returns {string} 字符串截取结果
         */
        subByte = string.subByte = function(source, length, tail) {
            source = String(source);
            tail = tail || '';
    ...
    (function() {
        ...
        calcHeightRevise = core.calcHeightRevise = function(style) {
            return flgContentBox ? toNumber(style.borderTopWidth) + toNumber(style.borderBottomWidth) +
                toNumber(style.paddingTop) + toNumber(style.paddingBottom) : 0;
        };
        ...
        calcWidthRevise = core.calcWidthRevise = function(style) {
            return flgContentBox ? toNumber(style.borderLeftWidth) + toNumber(style.borderRightWidth) +
                toNumber(style.paddingLeft) + toNumber(style.paddingRight) : 0;
        };
        ...
        triggerEvent = core.triggerEvent = function(control, name, event, args) {
            if (args && event) {
                ...
                event = {
                    returnValue: event,
                    preventDefault: UI_EVENT_CLASS.preventDefault
                };
                args = args || [];
            }
...
//{else}//
    ...
    UI_RADIO_CLASS.getItems = function () {
        ...
        if (!o) {
            ...
            list = form[o];
            //Modified By caijuan@baidu.com on 2013-4-15
            if (list && !list[0]) {  //只有一个选项的情况
                list = [];
                list.push(form[o]);
            }
            for (; o = list[i++]; ) {
...
//{else}//
     ...
     * @public
     *
     * @param {number} opacity 遮罩层透明度，默认为0.15
     */
    UI_FORM_CLASS.showModal = function (opacity) {
        if (!this._bFlag) {
            ...
            mask(opacity !== undefined ? opacity : 0.15, 32766 + UI_FORM_MODAL * 2);
